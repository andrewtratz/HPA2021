...
import cv2
import torch
from utils.puzzle_augment_utils import RandomResize
from config.config import *
from imgaug import augmenters as iaa

def puzzle_augment(image):
    image = train_multi_augment2(image)
    resizer = RandomResize(400, 600)
    image = resizer(image)
    return image

def uleft768(image):
    return image[:768, :768, :]
...
def augment_cuda_default(image):
    return image
...
def augment_cuda_flipud(image):
    return torch.flipud(image)
...
def augment_cuda_transpose(image):
    return torch.transpose(image, 2, 3)
...
def augment_cuda_flipud_lr(image):
    return torch.flipud(torch.fliplr(image))
...
def augment_cuda_flipud_transpose(image):
    return torch.transpose(torch.flipud(image), 2, 3)
...
def augment_cuda_fliplr_transpose(image):
    return torch.transpose(torch.fliplr(image), 2, 3)
...
def augment_cuda_flipud_lr_transpose(image):
    return torch.transpose(torch.fliplr(torch.flipud(image)), 2, 3)
